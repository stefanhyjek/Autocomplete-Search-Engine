...
The autocomplete system mainly uses binary search to find the first matching word and also uses a linear scan to collect remaining matches.

Runtime complexity:
O(log n + k) -> The binary search runs in: O(log ⁡n) where n is the number of words in the dictionary. The linear scan runs in: O(k) where k is the number of words that share the prefix.
